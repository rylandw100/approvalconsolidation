...
import { ACTION_TYPES, SELECT_ALL_CHIP } from '../Select.constants';
import type { FlatListItem, StateSelectedOption } from '../Select.types';
import Styled from './SelectControl.styles';
import type { SelectControlProps } from './SelectControl.types';
import { useCollapseIcon } from '../useCollapseIcon';
...
const SelectControl = (props: SelectControlProps) => {
  ...
  function renderCollapseIcon() {
    const { isDisabled: collapsedIconDisabled, isRotated, ...rest } = iconProps;
    return (
      canShowCollapseIcon() && (
        <Styled.CollapseIconContainer size={size} isMulti={isMulti} isRotated={isRotated}>
          <Icon {...rest} showCursor={false} isDisabled={collapsedIconDisabled} />
